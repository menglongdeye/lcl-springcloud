...
public class DepartFallBack implements DepartService {
    ...
    public List<Depart> findAll() {
        Depart arrays[] = {Depart.builder().id(8080).name("findAll错误8080").build(), Depart.builder().id(8080).name("findAll错误8080").build()};
        return Arrays.asList(arrays);
    }

    @Override
    public Depart query(int id) {
        return Depart.builder().id(8080).name("query错误8080").build();
    }
